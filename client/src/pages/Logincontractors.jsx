...
export default function ContractorregisterStep2() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      try {
        const res = await axiosInstance.post('/contractor/verify2ndstep', formData);
        if (res.status === 200) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2, bgcolor: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
        <Paper elevation={8} sx={{ p: 4, borderRadius: 6, width: '100%', maxWidth: 600 }}>
          ...
          <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Button variant="contained" component="label" fullWidth>
              Upload GST Document
              <input type="file" hidden name="gstDocument" onChange={handleFileChange} />
            </Button>
            {errors.gstDocument && <Typography color="error">{errors.gstDocument}</Typography>}

            <Button variant="contained" component="label" fullWidth>
              Upload License Document
              <input type="file" hidden name="licenseDocument" onChange={handleFileChange} />
            </Button>
