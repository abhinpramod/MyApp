...
const sendOtp = async (req, res) => {
  ...
  try {
    ...
    await sendEmail(email, 'Your OTP Code', `Your OTP is: ${otpCode}. This OTP is valid for 5 minutes.`);
...
const registerStore = async (req, res) => {
    try {
      ...
      if (!req.files || !req.files.gstDocument || !req.files.storeLicense) {
        return res.status(400).send('GST Document and Store License are required');
      }
  
      const { storeName, ownerName, country, state, city, address, email, phone, storeType, gstNumber,password } = req.body;
      ...
      if (existingStore) return res.status(400).send('Store already registered with this email');
      ...
      await newStore.save();
      await sendEmail(email, 'Registration Successful', 'Your store registration is awaiting admin approval.');
